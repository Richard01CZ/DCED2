      ...
      image9( (const char **) image9_data )
{
    ...
    ObjectWidgetLayout->addMultiCellWidget( extraBox, 2, 2, 0, 3 );
...
void ObjectWidget::languageChange()
{
	...
	lightHint = QString::null;

	sectorHint = QString::null;

	normalHint = trUtf8( "<font face=\"Arial\"><font size=\"+2\"><b>Help</b></font><br><br>"
						 "<font size=\"+1\"><b>I</b></font>n this dialog, you can change the general properties of an object. To change them, just click on the<i>Change</i> button in the lower right.<br>"
						 "<font size=\"+1\"><b>Y</b></font>ou can edit the name of an object in the line at the top, but also by selecting an item in the list at the left and pressing F2.<br>"
						 "<font size=\"+1\"><b>T</b></font>o change the position of an object, just edit the position values. There's nothing special on them. The same with the scaling values."
						 "<p><font size=\"+1\"><b>T</b></font>he rotation values are basically just like the position and scaling values, but there are some differences: The rotation unit is degree. That should make it easier for you to rotate an object. And another thing is, that changing the X and Y values for a human or a car doesn't effect anything because Mafia calculates the X and Y values for cars and humans itself. That's logic because a car always has to be on the ground with all of its four wheels. Mafia will always place it on the ground, even when you set the X- or Y-value to 180 degrees. The Z value of course does have an effect on cars and players, it changes the direction.</p>"
						 "<font size=\"+1\"><b>T</b></font>he model line shows the filename of the model this object loads.<br>"
						 "<font size=\"+1\"><b>T</b></font>he sector combobox shows a list of all sectors so you don't need to type the sector name in the line edit anymore. Of course, you still can do that, and the sector line will even help you with auto completion."
						 "</font>" );
}
...
void ObjectWidget::saveObject()
{
	if( edited == true )
	{
		QTextIStream( &objectNameInput->text() ) >> currentObject->sName;
		...
		{	
			...
			//lightSectorListBox->insertStringList( CHelper::lightSList2StringList( currentObject->sLightSectors ) );
		...
		else
		{
			...
			if( currentObject->bUseModel )
				QTextIStream( &modelLineEdit->text() ) >> currentObject->sModelName;
			...
			//DocumentWidget->DocumentOverviewWorkspace->DocumentOverview->MainWidget->statusBar()->message(...);
			//parent->parent->parent->parent->statusBar()->message( tr( "Object saved" ), 3000 );
		}; 
		emit saveObjectSignal( currentObject, oldName.latin1() );
	}; 
}

void ObjectWidget::setInputFields( ObjectInfo_t * newObject )
{
	if( currentObject != 0 )
	{
		saveObject();
	...
	//QMessageBox::information( 0, "Debug", QString::number( currentObject->lType ) );
	...
	if( currentObject->lType == 2 ) //Lights
	{
		...
		if( ( currentObject->lLightFlags&1 ) == 1 ) 
			flag1CheckBox->setChecked( true );
		else
			flag1CheckBox->setChecked( false );

		if( ( currentObject->lLightFlags&2 ) == 2 ) 
			flag2CheckBox->setChecked( true );
		else
			flag2CheckBox->setChecked( false );

		if( ( currentObject->lLightFlags&4 ) == 4 ) 
			flag3CheckBox->setChecked( true );
		else
			flag3CheckBox->setChecked( false );

		if( ( currentObject->lLightFlags&8 ) == 8 ) 
			flag4CheckBox->setChecked( true );
		else
			flag4CheckBox->setChecked( false );

		if( ( currentObject->lLightFlags&16 ) == 16 ) 
			flag5CheckBox->setChecked( true );
		else
			flag5CheckBox->setChecked( false );

		if( ( currentObject->lLightFlags&32 ) == 32 ) 
			flag6CheckBox->setChecked( true );
		else
			flag6CheckBox->setChecked( false );

		if( ( currentObject->lLightFlags&64 ) == 64 ) 
			flag7CheckBox->setChecked( true );
		else
			flag7CheckBox->setChecked( false );

		if( ( currentObject->lLightFlags&128 ) == 128 ) 
			flag8CheckBox->setChecked( true );
		else
			flag8CheckBox->setChecked( false );
		...
		for( int i = 0; i < lightSectorListBox->count(); i++ )
			debugList.append( lightSectorListBox->item( i )->text() );
		QMessageBox::information( 0, "Debug",
			QString::number( debugList.count() ) + "\n"
			+ QString::number( CHelper::lightSList2StringList( CHelper::lightStringList2SList( debugList ) ).count() ) );*/	
		...
		helpTextEdit->setText( lightHint );
	}
	else if( currentObject->lType == 153 ) //Sectors
	{
		if( positionGroupBox->isShown() )
		...
		helpTextEdit->setText( sectorHint );
	}
	else //All other objects
	{
		...
		helpTextEdit->setText( normalHint );
	}
